...
class Tick(CommandBase):
    ...
    async def _execute_chat(self, map_id, map_model, uid, entity_model, tuid, target_model, chats, topic=""):
        ...
        entity_model.set_action({"action": f"Talking to {target} about '{topic}'", "time": self.app.last_game_time, "target": target, "targetID": tuid})
        while len(chats) < 10:
    ...
    async def parse_react(self, result: Dict[str, Any], entity_model, map_id, map_model, uid):
        ...
        if "newPlan" in result["data"]:
            ...
            self.app.send(f"Player-{map_id}", {"code": 200, "uri": "newPlan", "uid": uid, "data": {"uid": uid, "plan": f'Go to {result["data"]["newPlan"]["building"]}. Purpose: {result["data"]["newPlan"]["purpose"]}'}})
            entity_model.save()
